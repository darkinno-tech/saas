...
func TestHTTPServeMuxExampleHandler(t *testing.T) {
	db, err := newDryRunDB()
	if err != nil {
	...
	newHandler(db, tenants).ServeHTTP(recorder, request)
...
func TestHTTPServeMuxExampleHandlerRejectsInvalidTenantRequests(t *testing.T) {
	db, err := newDryRunDB()
	if err != nil {
		t.Fatalf("newDryRunDB() error = %v", err)
	}
